...
class JournalAcronIdFile(CommonControlField, ClusterableModel):
    ...
    collection_journal = models.ForeignKey(
...
class IdFileRecord(CommonControlField, Orderable):
    ...
    def create(
        ...
        if not user and not item_type and not parent and not item_pid:
            ...
            raise ValueError(
                f"IdFileRecord.create requires all parameters. Got {d}"
            )
    ...
    def get_record_data(self, journal_acron_data=None, issue_data=None):
        data = {}
        data["title"] = journal_acron_data
        if not issue_data:
            issue_data = (
